...
exports.logSafetyEvent = exports.assessSafetyRisk = void 0;
...
const assessSafetyRisk = (message) => {
    const hasImminentRisk = imminentRiskPatterns.some((pattern) => pattern.test(message));
    if (hasImminentRisk) {
        return {
            hasCrisisRisk: true,
            riskLevel: "imminent",
    ...
    const hasPossibleRisk = possibleRiskPatterns.some((pattern) => pattern.test(message));
    if (hasPossibleRisk) {
        return {
            hasCrisisRisk: true,
            riskLevel: "possible",
...
exports.assessSafetyRisk = assessSafetyRisk;
const logSafetyEvent = async ({ userId, eventType, riskLevel, source, message, }) => {
